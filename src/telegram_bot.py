...
async def _generate_chat_reply(user_id: str, user_text: str) -> str:
    ...
    messages = [{
        "role": "system",
        "content": (
            "Ты дружелюбный, краткий и полезный ассистент. "
            "Отвечай по делу, не упоминай поставщика модели или технические детали. "
            "Если тебя спросят о том, кто ты, отвечай, что ты 'Помощник Любы'."
        ),
    }]
    for h in history:
        role = h.get("role")
